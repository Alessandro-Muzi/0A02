...
La porta 8080 era già occupata da Apache, che su questa stessa macchina serve anche la staff interface di Koha. Il fix richiedeva cambiare la porta di Headscale, ma il primo tentativo è stato incompleto: aggiornata solo `headscale_server_url` a `8443` in `group_vars/all.yml`, lasciando invariato `headscale_listen_port` (ancora al default `8080`). Il servizio continuava a fallire con lo stesso errore.
...
listen_addr: 127.0.0.1:8080   # non ancora aggiornato
```

Fix completo, allineando entrambe le variabili:
